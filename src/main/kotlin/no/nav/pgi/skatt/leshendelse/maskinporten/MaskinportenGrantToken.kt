...
internal class MaskinportenGrantToken(env: Map<String, String>) {
    init {
        env.verify(listOf(PRIVATE_JWK_ENV_KEY, AUDIENCE_ENV_KEY, ISSUER_ENV_KEY, SCOPE_ENV_KEY, VALID_IN_SECONDS_ENV_KEY))
    }

    private val privateKey: RSAKey = RSAKey.parse(env.getVal(PRIVATE_JWK_ENV_KEY))
    private val jwsSigner: JWSSigner = RSASSASigner(privateKey)
